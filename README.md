 ```
ВАЖНЫЙ МОМЕНТ РАСПАКУЙТЕ АРХИХ И ПАПКУ КОТОРУЮ УВИДИТЕ В НЕЙ ПЕРЕКИНТЕ НА РАБОЧИЙ СТОЛ И ТОЛЬКО ТАМ ОТКРЫВАЙТЕ (через Index.html)
🛠 Установка и настройка
 ...
 Откройте файл `index.html` в удобном вами редактор кода или обычный блокнот и настройте под себя:
